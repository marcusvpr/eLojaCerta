...
from b_comentarios.forms import FormComentario
from b_comentarios.models import B_Comentario
...
class PostIndex(ListView):
    model = B_Post
    template_name = 'b_posts/index.html'
    paginate_by = 6
    ...
    def get_queryset(self):
        ...
        qs = qs.annotate(
            numero_comentarios=Count(
                Case(
                    When(b_comentario__publicado_comentario=True, then=1)
                )
            )
        )
...
class PostBusca(PostIndex):
    template_name = 'b_posts/post_busca.html'
...
class PostCategoria(PostIndex):
    template_name = 'b_posts/post_categoria.html'
...
class PostDetalhes(View):
    template_name = 'b_posts/post_detalhes.html'

    def setup(self, request, *args, **kwargs):
        super().setup(request, *args, **kwargs)

        pk = self.kwargs.get('pk')
        post = get_object_or_404(B_Post, pk=pk, publicado_post=True)
        self.contexto = {
            'post': post,
            'comentarios': B_Comentario.objects.filter(post_comentario=post,
